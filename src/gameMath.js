...
export function buildRiskTable(difficulty = 'medium', rtp = RTP_TARGET) {
  return Array.from({ length: MAX_CROSSINGS }, (_, index) => {
    const n = index + 1;
    return { n, greenProbability: getGreenProbability(n, difficulty), cumulativeProbability: getCumulativeProbability(n, difficulty), totalMultiplier: getMultiplier(n, difficulty, rtp) };
  });
}
