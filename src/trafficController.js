...
export class IntersectionTraffic {
  constructor(index, getMaterial) {
    this.root = new THREE.Group(); this.root.name = 'cross-traffic';
  ...
  update(dt, open, reducedMotion = false, emergency = false) {
    for (const car of this.cars) {
      ...
      if (emergency) {
        const exiting=car.progress>STOP_LINE,edge=(exiting?1:-1)*(LOOP_HALF+3);
        car.progress=reducedMotion?edge:exiting?Math.min(edge,car.progress+Math.max(0,dt)*42):Math.max(edge,car.progress-Math.max(0,dt)*24);
      }
      ...
      car.root.position.set(-car.direction * 1.35, .03, MAIN_ROAD_Z + car.direction * car.progress);
