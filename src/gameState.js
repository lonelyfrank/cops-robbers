import { DIFFICULTIES, MAX_CROSSINGS, calculatePayout, getMultiplier, isGreen } from './gameMath.js';
...
export class GameState {
  ...
  notify() { const snapshot = this.snapshot; this.listeners.forEach(listener => listener(snapshot)); }
  get canConfigure() { return [PHASES.IDLE, PHASES.RESULT].includes(this.data.phase); }
  setBet(amount) {
    if (!this.canConfigure || !Number.isSafeInteger(amount) || amount < MIN_BET || amount > MAX_BET || amount > this.data.balance) return false;
    this.data.bet = amount; this.notify(); return true;
  }
  setDifficulty(difficulty) {
    if (!this.canConfigure || !Object.hasOwn(DIFFICULTIES, difficulty)) return false;
    this.data.difficulty = difficulty; this.notify(); return true;
  }
  ...
  cashout() {
    ...
    if (!Number.isSafeInteger(this.data.balance + payout)) throw new RangeError('Saldo fuori limite.');
    Object.assign(this.data, { phase: PHASES.ESCAPING, payout, balance: this.data.balance + payout });
    this.record('won'); this.notify(); return true;
  }
  finishCaught() {
    if (this.data.phase !== PHASES.CAUGHT) return false;
    this.data.phase = PHASES.RESULT;
    this.record('lost'); this.notify(); return true;
  }
  finishEscape() {
    if (this.data.phase !== PHASES.ESCAPING) return false;
    this.data.phase = PHASES.RESULT; this.notify(); return true;
  }
  record(outcome) {
    this.data.history.unshift({ id: this.data.round, outcome, difficulty: this.data.difficulty, crossing: this.data.crossing, attemptedCrossing: outcome === 'lost' ? this.data.crossing + 1 : this.data.crossing, multiplier: this.data.multiplier, stake: this.data.stake, payout: this.data.payout });
