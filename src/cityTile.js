...
export function createCityTile(index, { lighting = createCityLighting() } = {}) {
  ...
  const getMaterial = (key) => {
    if (!records.has(key)) {
      ...
      records.set(key, {
        material: m,
        base: new THREE.Color(config.color),
        power: config.power ?? 0,
        kind: config.power ? 'warm' : 'solid',
      });
    }
  ...
  for (let z = 8.5; z < 13; z += 0.6) batch.add(0xc9b994, [1.02, 0.035, 0.57], [-4.7, 0.274, z]);
  ...
  tile.traffic = new IntersectionTraffic(index, getMaterial);
  objectRoot.add(tile.traffic.root);
  ...
  tile.update = (dt, progress, { reducedMotion = false, trafficBlocked = false } = {}) => {
    ...
    const focus = lighting.focusX.value,
  ...
  tile.dispose = () => {
    ...
    signals.forEach((signal) =>
      Object.values(signal.lamps).forEach((lamp) => lamp.material.dispose()),
    );
...
export function disposeTileSharedAssets() {
  haloTexture.dispose();
}
