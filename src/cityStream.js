...
export class CityStream {
  constructor(scene,{reducedMotion=false,createTile=createCityTile}={}) {
    ...
    this.lighting=createCityLighting();this.playerX=getStopX(0);
    this.tiles=new Map();this.current=1;this.signals=new Map([[1,'red']]);
  ...
  setMovement(crossing) {
    ...
    this.signals.clear();
    if(crossing!==null){this.signals.set(crossing,'green');this.signals.set(crossing-1,'green');}
    for(const [index,{tile}] of this.tiles)tile.setSignal(this.signals.get(index)??'red');
  }
  setCaught(value){this.caught=Boolean(value);}
  setPlayerX(x){const next=getCurrentTile(x);this.playerX=x;if(next!==this.current)this.sync(next);}
  sync(current,immediate=false) {
    ...
    for(const index of wanted) {
      if(!this.tiles.has(index)) {
        const tile=this.createTile(index,{lighting:this.lighting});const reveal=immediate||this.reducedMotion?1:0;
        tile.setSignal(this.signals.get(index)??'red');this.scene.add(tile.root);
  ...
  update(dt) {
    ...
    this.lighting.focusX.value+=(focus-this.lighting.focusX.value)*(this.reducedMotion?1:1-Math.exp(-Math.max(0,dt)*6));
    this.lighting.capture.value+=((this.caught?1:0)-this.lighting.capture.value)*(this.reducedMotion?1:1-Math.exp(-Math.max(0,dt)*8));
    for(const [index,entry] of this.tiles) {
      ...
      entry.tile.update(dt,entry.reveal,{reducedMotion:this.reducedMotion,trafficBlocked:this.caught});
    }
  }
  reset() {
    for(const {tile} of this.tiles.values())tile.dispose();this.tiles.clear();
    this.playerX=getStopX(0);this.lighting.focusX.value=0;this.lighting.capture.value=0;
    this.caught=false;this.signals.clear();this.signals.set(1,'red');this.sync(1,true);
  }
  dispose() {
    if(this.disposed)return;
