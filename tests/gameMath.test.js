...
test('All difficulties start at 95%; probabilities decrease and multipliers increase', () => {
  for (const difficulty of Object.keys(DIFFICULTIES)) {
    close(getGreenProbability(1, difficulty), .95);
...
test('RTP can be recalibrated independently of risk and rendering', () => {
  for (const target of [.9, .96, .98, 1]) for (const d of Object.keys(DIFFICULTIES)) {
    for (const row of buildRiskTable(d, target)) close(row.cumulativeProbability * row.totalMultiplier, target);
  }
});

test('Payouts settle in integer hundredths, never round up, and differ by less than 0.01 CR', () => {
  for (const d of Object.keys(DIFFICULTIES)) for (let n = 1; n <= MAX_CROSSINGS; n++) for (const stake of [100, 123, 2500, 100000, 100000000]) {
    const exact = stake * getMultiplier(n, d), actual = calculatePayout(stake, n, d);
    assert.ok(Number.isSafeInteger(actual)); assert.ok(actual <= exact + 1e-8); assert.ok(exact - actual < 1);
...
test('Green uses a strict random boundary and rejects invalid samples', () => {
  ...
  assert.equal(isGreen(1, 'easy', 0), true); assert.equal(isGreen(1, 'easy', p - 1e-9), true);
  assert.equal(isGreen(1, 'easy', p), false); assert.equal(isGreen(1, 'easy', .99999), false);
  for (const v of [-.1, 1, NaN, Infinity, '0.5']) assert.throws(() => isGreen(1, 'easy', v), RangeError);
});

test('Math rejects invalid crossing, difficulty, RTP, and unsafe monetary input', () => {
  for (const n of [-1, .5, 13, NaN]) assert.throws(() => getMultiplier(n), RangeError);
  for (const d of ['unknown', 'constructor', '__proto__']) assert.throws(() => getMultiplier(1, d), RangeError);
  assert.throws(() => getGreenProbability(0), RangeError);
  for (const rtp of [0, -1, 1.01, NaN]) assert.throws(() => getMultiplier(1, 'easy', rtp), RangeError);
  for (const stake of [0, -1, .5, NaN, Number.MAX_SAFE_INTEGER]) assert.throws(() => calculatePayout(stake, 12, 'hard'), RangeError);
  close(getCumulativeProbability(0), 1); close(getMultiplier(0), 1);
});
