...
test('The running animation reaches the next safe stop and invokes completion once', () => {
  const scene = new THREE.Scene(),
    c = new CharacterController(scene);
  let completed = 0;
  c.run(1, () => completed++);
  finish(c);
  assert.equal(completed, 1);
  assert.equal(c.thief.root.position.x, getStopX(1));
  assert.equal(c.thief.root.visible, true);
...
test('Arrest introduces a police car and raised hands; reset restores all props', () => {
  ...
  finish(c);
  assert.equal(completed, 1);
  assert.equal(c.police.root.visible, true);
  assert.ok(c.thief.arms[0].rotation.z < -2);
  c.reset();
  assert.equal(c.police.root.visible, false);
  assert.equal(c.police.blueLight.intensity, 0);
  assert.equal(c.thief.root.position.x, getStopX(0));
  assert.equal(c.thief.arms[0].rotation.z, 0);
});
...
test('Alley cashout turns the thief off the avenue and completes once', () => {
  ...
  assert.ok(c.thief.root.position.z > 9);
});
...
test('Cashout uses the alley at every crossing in normal and reduced motion', () => {
  for (const reducedMotion of [false, true])
    for (let n = 1; n <= 12; n++) {
      const scene = new THREE.Scene(),
        c = new CharacterController(scene, { reducedMotion });
      let completed = 0;
      ...
      c.reset();
      assert.equal(c.thief.root.visible, true);
      assert.equal(c.thief.root.position.z, RUNNER_Z);
    }
});
...
test('Running reacts on the first frame and settles its pose before stopping', () => {
  ...
  c.update(duration - 1 / 60 - 0.01);
  assert.ok(c.animation);
  assert.ok(Math.abs(c.thief.legs[0].rotation.z) < 0.01);
  assert.ok(Math.abs(c.thief.body.rotation.z) < 0.01);
  finish(c, 0.1);
  assert.equal(c.thief.root.position.x, getStopX(1));
...
test('The patrol waits still, then advances alongside the thief from the first frame', () => {
  ...
  c.update(0.5);
  assert.equal(c.chase.root.position.x, startX);
  c.run(1);
  c.update(1 / 60);
...
test('The arrest car approaches head-on and stops in front without touching the thief', () => {
  const c = new CharacterController(new THREE.Scene());
  c.caught(1);
  const startX = c.police.root.position.x;
  c.update(0.1);
...
test('Loot grows smoothly at every multiplier, stays bounded and resets for a new round', () => {
  for (const reducedMotion of [false, true]) {
    ...
    c.reset();
    assert.equal(c.thief.lootBag.scale.x, 1);
    assert.equal(c.lootTarget, 1);
  }
});
...
test('Capture cars enter from street ends and form a four-sided ring around the thief', () => {
  for (const reducedMotion of [false, true]) {
    ...
    finish(c, 2.5);
    const thief = c.thief.root.position,
      [north, south] = c.flankPolice;
